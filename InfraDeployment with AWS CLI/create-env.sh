...
aws s3api put-bucket-policy --bucket ${19} --policy file://policy.json
...
aws s3api put-bucket-policy --bucket ${20} --policy file://policy.json
...
RDS_Address=$(aws rds describe-db-instances --db-instance-identifier ${13} --query "DBInstances[0].Endpoint.Address")
